...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        loss += 0.5*self.reg*(np.sum(self.params["W1"]**2)+np.sum(self.params["W2"]**2))  #reg
        dout, dw2, db2 = affine_backward(dout, out_cache)
        dw2 += self.reg*self.params["W2"]                      #reg
        grads["W2"], grads["b2"] = dw2, db2
        _, dw1, db1 = affine_relu_backward(dout, h1_cache)
        dw1 += self.reg*self.params["W1"]                      #reg
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for idx in range(self.num_layers-1):
            lay = self.num_layers - 1 - idx - 1    # Calculate layer_num
